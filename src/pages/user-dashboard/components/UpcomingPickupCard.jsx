...
const UpcomingPickupCard = ({ pickup, onMessage }) => {
    ...
    >
      <div className="flex items-start gap-3 md:gap-4 mb-3 md:mb-4">
        <div className="w-12 h-12 md:w-14 md:h-14 rounded-lg overflow-hidden flex-shrink-0">
          <Image
            src={pickup?.courierImage}
            alt={pickup?.courierImageAlt}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <span className="text-xs md:text-sm font-medium text-primary">
              Pickup in {getTimeUntil(pickup?.scheduledTime)}
            </span>
          </div>
        </div>
      </div>
      <div className="space-y-2 md:space-y-3 mb-3 md:mb-4">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs md:text-sm text-foreground">{pickup?.packageType}</p>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2 pt-3 md:pt-4 border-t border-border">
        ...
        <button
          ...
          onClick={() => onMessage?.(pickup)}
          disabled={!pickup?.courierName || pickup?.courierName === 'Unassigned'}
        >
